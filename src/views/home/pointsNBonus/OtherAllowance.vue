...
import { type OtherAllowance, ReviewState, deleteOtherAllowance, getOtherAllowance, getOtherAllowanceList } from '@/api';
import dayjs from 'dayjs';
import OtherAllowanceForm from '@/components/home/pointsNbonus/OtherAllowanceForm.vue';
...
const list = ref<(OtherAllowance & { fileType: null | FileType })[]>([]);
...
async function getReviewList() {
  ...
  const { data, meta } = await getOtherAllowanceList({ ...(userId && { userId }), page: pagination.value.page, take: pagination.value.rowsPerPage });
...
// upload
const targetAllowance = ref();
const bonusInfo = ref<any>(null);
const role = computed(() => userStore.role);

async function editReview(id: number) {
  const data = await getOtherAllowance(id);
  targetAllowance.value = data.id;
  bonusInfo.value = {
    title: data.title,
    userId: data.user.id,
    amount: data.amount,
...
function uploadReview() {
  ...
  $q.notify({ message: `其他津貼${formType.value === 'add' ? '上傳' : '編輯'}成功`, timeout: 600, position: 'top' });
  getReviewList();
}
...
function deleteConfirm(id: number) {
  $q.dialog({
    ...
    try {
      await deleteOtherAllowance(id);
      $q.notify({ message: '其他津貼已刪除', timeout: 600, position: 'top' });
      getReviewList();
    }
    catch (err) {
      console.log(err);
...
<template>
  <div>
    <section class="row items-end q-py-md">
      <div>
        <h3 class="text-headline-medium q-mb-md">其他津貼管理</h3>
    ...
    <section>
      <div class="q-py-md">
        <OptionSelect v-if="userStore.canI('EDIT_OTHER_ALLOWANCE')" v-model="selectedTherapist" :options="bonusStore.therapistFilterOptions" @update:model-value="getReviewList" />
      </div>
      <QTable
      ...
      >
        ...
        <template #body-cell-action="{ row }">
          <QTd auto-width>
            <QBtn v-if="userStore.canI('EDIT_OTHER_ALLOWANCE') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_delete" class="q-mr-sm" @click="deleteConfirm(row.id)" />
            <QBtn v-if="userStore.canI('EDIT_OTHER_ALLOWANCE') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_edit" @click="editReview(row.id)" />
          </QTd>
        </template>
      </QTable>
    </section>
