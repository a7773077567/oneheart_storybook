...
import { type RelocationBonus, ReviewState, deleteExpatAllowance, getExpatAllowance, getExpatAllowanceList } from '@/api';
...
const list = ref<(RelocationBonus & { fileType: null | FileType })[]>([]);
...
async function getReviewList() {
  ...
  const { data, meta } = await getExpatAllowanceList({ ...(userId && { userId }), page: pagination.value.page, take: pagination.value.rowsPerPage });
...
async function editReview(id: number) {
  const data = await getExpatAllowance(id);
...
function deleteConfirm(id: number) {
  $q.dialog({
    ...
    try {
      await deleteExpatAllowance(id);
      $q.notify({ message: '外派獎金已刪除', timeout: 600, position: 'top' });
      getReviewList();
    }
    catch (err) {
      console.log(err);
...
<template>
  <div>
    <section class="row items-end q-py-md">
      <div>
        <h3 class="text-headline-medium q-mb-md">外派獎金管理</h3>
    ...
    <section>
      <div class="q-py-md">
        <OptionSelect v-if="userStore.canI('EDIT_RELOCATE_BONUS')" v-model="selectedTherapist" :options="bonusStore.therapistFilterOptions" @update:model-value="getReviewList" />
      </div>
      <QTable
      ...
      >
        ...
        <template #body-cell-action="{ row }">
          <QTd auto-width>
            <QBtn v-if="userStore.canI('EDIT_RELOCATE_BONUS') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_delete" class="q-mr-sm" @click="deleteConfirm(row.id)" />
            <QBtn v-if="userStore.canI('EDIT_RELOCATE_BONUS') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_edit" @click="editReview(row.id)" />
          </QTd>
        </template>
      </QTable>
    </section>
