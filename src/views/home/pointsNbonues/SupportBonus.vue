...
import { ReviewState, type SupportBonus, deleteSupportBonus, getSupportBonus, getSupportBonusList } from '@/api';
...
const list = ref<(SupportBonus & { fileType: null | FileType })[]>([]);
...
function uploadReview() {
  ...
  if (formType.value === 'add') {
    ...
    $q.notify({ message: `支援獎金編輯成功`, timeout: 600, position: 'top' });
  }
  getReviewList();
}
...
<template>
  <div>
    ...
    <section>
      <div class="q-py-md">
        <OptionSelect v-if="userStore.canI('EDIT_SUPPORT_BONUS')" v-model="selectedTherapist" :options="bonusStore.therapistFilterOptions" @update:model-value="getReviewList" />
      </div>
      <QTable
      ...
      >
        ...
        <template #body-cell-action="{ row }">
          <QTd auto-width>
            <QBtn v-if="userStore.canI('EDIT_SUPPORT_BONUS') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_delete" class="q-mr-sm" @click="deleteConfirm(row.id)" />
            <QBtn v-if="userStore.canI('EDIT_SUPPORT_BONUS') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_edit" @click="editReview(row.id)" />
          </QTd>
        </template>
      </QTable>
    </section>
