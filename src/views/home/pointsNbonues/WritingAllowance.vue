...
import { ReviewState, type WritingAllowance, deleteWritingAllowance, getWritingAllowance, getWritingAllowanceList } from '@/api';
import dayjs from 'dayjs';
import WritingAllowanceForm from '@/components/home/pointsNbonus/WritingAllowanceForm.vue';
...
const list = ref<(WritingAllowance & { fileType: null | FileType })[]>([]);
...
async function getReviewList() {
  ...
  const { data, meta } = await getWritingAllowanceList({ ...(userId && { userId }), page: pagination.value.page, take: pagination.value.rowsPerPage });
...
async function editBonus(id: number) {
  const data = await getWritingAllowance(id);
...
function uploadReview() {
  ...
  $q.notify({ message: `寫作津貼${formType.value === 'add' ? '上傳' : '編輯'}成功`, timeout: 600, position: 'top' });
  getReviewList();
}
...
function deleteConfirm(id: number) {
  $q.dialog({
    ...
    try {
      await deleteWritingAllowance(id);
      $q.notify({ message: '寫作津貼已刪除', timeout: 600, position: 'top' });
      getReviewList();
    }
    catch (err) {
      console.log(err);
...
<template>
  <div>
    <section class="row items-end q-py-md">
      <div>
        <h3 class="text-headline-medium q-mb-md">寫作津貼管理</h3>
    ...
    <section>
      <div class="q-py-md">
        <OptionSelect v-if="userStore.canI('EDIT_WRITING_ALLOWANCE')" v-model="selectedTherapist" :options="bonusStore.therapistFilterOptions" @update:model-value="getReviewList" />
      </div>
      <QTable
      ...
      >
        ...
        <template #body-cell-action="{ row }">
          <QTd auto-width>
            <QBtn v-if="userStore.canI('EDIT_WRITING_ALLOWANCE') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_delete" class="q-mr-sm" @click="deleteConfirm(row.id)" />
            <QBtn v-if="userStore.canI('EDIT_WRITING_ALLOWANCE') && (row.status === ReviewState['待審核'] || userStore.canI('EDIT_REVIEW'))" flat round icon="o_edit" @click="editBonus(row.id)" />
          </QTd>
        </template>
      </QTable>
    </section>
