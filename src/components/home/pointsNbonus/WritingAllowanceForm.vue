...
import { z } from 'zod';
import { createWritingAllowance, getWritingAllowanceUploadURL, updateWritingAllowance, upload2awsS3 } from '@/api';
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    if (newAttachment.value) {
      const { fileName, url, maxFileSizeInMB } = await getWritingAllowanceUploadURL({ userId: values.userId });
    ...
    if (props.type === 'edit' && !!props.bonusId) {
      await updateWritingAllowance(props.bonusId, {
...
<template>
  <QCard class="form">
    <QCardSection class="q-pa-lg">
      <h2 class="text-headline-small">{{ type === 'add' ? '上傳寫作津貼' : '編輯寫作津貼' }}</h2>
    </QCardSection>
