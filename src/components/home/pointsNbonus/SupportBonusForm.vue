...
import { z } from 'zod';
import { createSupportBonus, getSupportBonusUploadURL, updateSupportBonus, upload2awsS3 } from '@/api';
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    if (newAttachment.value) {
      const { fileName, url, maxFileSizeInMB } = await getSupportBonusUploadURL({ userId: values.userId });
    ...
    if (props.type === 'edit' && !!props.bonusId) {
      await updateSupportBonus(props.bonusId, {
...
<template>
  <QCard class="form">
    <QCardSection class="q-pa-lg">
      <h2 class="text-headline-small">{{ type === 'add' ? '上傳支援獎金' : '編輯支援獎金' }}</h2>
    </QCardSection>
