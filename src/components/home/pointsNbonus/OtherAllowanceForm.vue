...
import { z } from 'zod';
import { createOtherAllowance, getOtherAllowanceUploadURL, updateOtherAllowance, upload2awsS3 } from '@/api';
...
const onSubmit = handleSubmit(async (values) => {
  ...
  try {
    if (newAttachment.value) {
      const { fileName, url, maxFileSizeInMB } = await getOtherAllowanceUploadURL({ userId: values.userId });
    ...
    if (props.type === 'edit' && !!props.bonusId) {
      await updateOtherAllowance(props.bonusId, {
...
<template>
  <QCard class="form">
    <QCardSection class="q-pa-lg">
      <h2 class="text-headline-small">{{ type === 'add' ? '上傳其他津貼' : '編輯其他津貼' }}</h2>
    </QCardSection>
