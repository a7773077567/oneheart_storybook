...
<template>
  <div class="client-info">
    ...
    <div class="client-info__actions">
      <div class="actions">
        <QBtn v-if="!isCheckedOut && canCheckout" :disable="!appointmentStore.isSameClinicSchedule" class="actions__item--checkout" label="結帳" icon="attach_money" color="primary" style="width: 127px;" @click="$router.push({ name: 'appointmentListCheckout', params: { scheduleId: schedule.id } })" />
        <QBtn class="actions__item--rearrange" label="預約改期" :disable="schedule.state > 2 || !appointmentStore.isSameClinicSchedule" outline style="width: 127px;" @click="rearrangeClientSchedule" />
        <QBtn class="actions__item--cancel" label="取消預約" :disable="schedule.state > 2 || !appointmentStore.isSameClinicSchedule" color="red-10" style="width: 127px;" @click="cancelClientSchedule" />
